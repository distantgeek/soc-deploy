...
**M3 (Wazuh) DEFERRED ~2026-10-01:** monthly API usage at 95% (resets ~Oct 1). Do NOT start Wazuh deployment until after the reset. Malcolm M0–M2 are complete; SO is decommissioned (VM 300 stopped, kept as backup). `so-capture` is on soc-host (`/usr/local/sbin/so-capture`, ens19 interface).
